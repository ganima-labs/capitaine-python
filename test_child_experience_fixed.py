#!/usr/bin/env python3
"""
Test complet de l'expérience Capitaine Python - Simulation apprenant de 9 ans (CORRIGÉ)
"""
...
class ChildExperienceTester:
    ...
    def __init__(self):
        ...
        self.child_name = "Léo"  # Nom d'enfant de 9 ans
        self.progress = []
    ...
    def generate_child_inputs(self, code: str) -> list:
        ...
        child_inputs = ["Léo", "9", "bonjour", "rouge", "42", "chat", "python"]
    ...
    def run_complete_child_test(self):
        ...
        # 3. Récupérer les cours disponibles
        courses = self.get_available_courses()
        if not courses:
            return False

        # 4. Choisir un cours adapté
        course_id = self.choose_course_for_child(courses)
        if not course_id:
            return False

        # 5. Récupérer les exercices
        exercises = self.get_course_exercises(course_id)
        if not exercises:
            return False

        # 6. Tester quelques exercices (limité à 5 pour le test)
        ...
        # 7. Tester le suivi de progression
        ...
        self.generate_report(results, success_count, len(test_exercises))

        return success_count >= len(test_exercises) * 0.7

    def generate_report(self, results: list, success_count: int, total_count: int):
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("La plateforme est prête pour les enfants. 🚀")
    else:
